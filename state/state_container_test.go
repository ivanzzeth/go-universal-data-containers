...
func SpecTestFinalizerStateContainer(t *testing.T, cache, persist Storage, finalizer Finalizer, lockerGenerator locker.SyncLockerGenerator) {
	t.Run("Single instance access states", func(t *testing.T) {
		user1Container := NewStateContainerWithFinalizer(finalizer, MustNewTestUserModel(lockerGenerator, "partition1", "user1", "server"))
		...
		newUser1, err := NewStateContainerWithFinalizer(finalizer, MustNewTestUserModel(lockerGenerator, "partition1", "user1", "server")).Get(context.Background())
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("Multiple instances access states", func(t *testing.T) {
		user2Container := NewStateContainerWithFinalizer(finalizer, MustNewTestUserModel(lockerGenerator, "partition1", "user2", "serve2"))
		user3Container := NewStateContainerWithFinalizer(finalizer, MustNewTestUserModel(lockerGenerator, "partition1", "user3", "serve2"))
